...
class RetailSalesAnalyzer:
    """
    Analyzes US retail sales for macroeconomic intelligence.

    NIFDA retrieves:
    - Latest retail sales
    - Retail sales approximately 12 months earlier
    - Retail sales approximately 24 months earlier
    ...
    def _get_value(observation: dict) -> float:
        ...
        if value in (None, "", "."):
            raise ValueError("Retail sales observation has no valid value.")
    ...
    def _get_latest_observation(self) -> dict:
        ...
        if not observations:
            raise ValueError("No latest retail sales observation returned.")
    ...
    def analyze_yoy(
        ...
        """
        Calculate current retail sales year-over-year growth.
        """
        ...
        if result.direction == "RISING":
            ...
            demand_view = (
                "Consumer demand is broadly stable."
            )

        if momentum == "ACCELERATING":
            momentum_view = (
                "Retail sales growth is accelerating."
            )
        elif momentum == "DECELERATING":
            momentum_view = (
                "Retail sales growth is decelerating."
            )
        else:
            momentum_view = (
                "Retail sales growth is broadly stable."
            )
